...
/**
 ...
 @details the mpart::Initialize function will set add this function to the
 `atexit` list so that it is called at program termination.
 */
void Finalize();

// Simply holds a private variable isInitialized.  A static instance of this
// function is used in the GetInitializeStatusObject object.
...
/**
 ...
 @ingroup InitializationHelpers
 @details
 This function provides a thin wrapper around the Kokkos::initialize function
 that can be called multiple times without error. This function also adds
 Kokkos::finalize to `atexit` so that manually calling Kokkos::finalize is not
 necessary.

 Note that Kokkos::initialize can only be called once and this function will
 print a warning message if called multiple times.  The warning indicates that
 any changes to Kokkos parameters, like `--kokkos-num-threads` in subsequent
 calls will have no impact.  Only the parameters passed the to the first
 mpart::Initialize call will be used.

 The warning messages can be silenced by setting the `MPART_WARNINGS`
...
void Initialize(FirstArgType& arg1, Arguments... args) {
  if (!GetInitializeStatusObject().Get()) {
    ...
    if (warningStr != nullptr) {
      ...
      // Convert to lowercase so we catch OFF, off, Off, or any combination of
      // capitalization.
      std::transform(warningFlag.begin(), warningFlag.end(),
                     warningFlag.begin(),
                     [](unsigned char c) { return std::tolower(c); });

      if (warningFlag == "off") shouldPrint = false;
    }

    if (shouldPrint)
      std::cout
          << "WARNING: mpart::Initialize has already been called.  Any "
             "changes to runtime settings (e.g., \"--kokkos-num-threads\") "
